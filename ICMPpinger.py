...
def receiveOnePing(mySocket, ID, timeout, destAddr): 
    ...
    while True:
        ...
        # use struct to unpack the string
        request_type, code, rev_checksum, rev_id, rev_sequence = struct.unpack('bbHHh', icmp_header)
        if ID == rev_id:
            bytes_in_double = struct.calcsize('d')
            time_data = struct.unpack('d', rec_packet[28:28 + bytes_in_double])[0]
            return timeReceived - time_data
        else:
